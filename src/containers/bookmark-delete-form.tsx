...
const BookmarkDeleteForm: FC = () => {
  ...
  const shouldDisplay = useSelector(
    state => state.bookmarks.displayDeleteBookmarkModal,
  )
  ...
  return (
    <Modal>
      <header>
        <Heading>
          Delete bookmark <em>{bookmarkTitle}</em>?
        </Heading>
      </header>

      <Button
        onClick={(): void =>
          void dispatch(setDeleteBookmarkModalDisplay(false))
        }
      >
        Cancel
      </Button>
      <ConfirmationButton onClick={(): void => void dispatch(deleteBookmark())}>
        Delete
      </ConfirmationButton>
    </Modal>
  )
