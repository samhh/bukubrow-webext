...
const BookmarkForm: FC<Props> = props => {
  ...
  const setInputBookmarkPartial = (
    partialBookmark: Partial<BookmarkInput>,
  ): void => {
  ...
  const handleBookmarkTextInput = (key: KeyofStringValues<BookmarkInput>) => (
    input: string,
  ): void => {
  ...
  return (
    <>
      ...
      <Wrapper>
        <Header>
          <Heading>{isEditing ? "Edit bookmark" : "Add a bookmark"}</Heading>
        </Header>
        ...
        <TagList>
          {bookmarkInput.tags.map(tag => (
            <Tag key={tag} id={tag} label={tag} onRemove={handleTagRemoval} />
          ))}
        </TagList>
