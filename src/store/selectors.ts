...
import {
  filterBookmark,
  ordLocalBookmarkWeighted,
  LocalBookmark,
  LocalBookmarkWeighted,
...
import {
  ...
  ordStagedBookmarksGroup,
  bookmarks,
} from "~/modules/staged-groups"
import { values } from "fp-ts-std/Record"
import * as S from "fp-ts-std/String"

const addBookmarkWeight = (activeTabURL: Option<URL>) => (
...
const getFocusedBookmarkIndex = (state: AppState) =>
  state.bookmarks.focusedBookmarkIndex
const getBookmarkEditId = (state: AppState) => state.bookmarks.bookmarkEditId
const getBookmarkDeleteId = (state: AppState) =>
  state.bookmarks.bookmarkDeleteId
const getLimitNumRendered = (state: AppState) =>
  state.bookmarks.limitNumRendered
const getStagedGroups = (state: AppState) =>
  state.bookmarks.stagedBookmarksGroups
const getStagedGroupEditId = (state: AppState) =>
  state.bookmarks.stagedBookmarksGroupEditId
const getStagedGroupBookmarkEditId = (state: AppState) =>
  state.bookmarks.stagedBookmarksGroupBookmarkEditId
 ...
 */
export const getUnlimitedFilteredBookmarks = createSelector(
  getBookmarks,
  getParsedFilter,
  (bookmarks, inputFilter) =>
    pipe(bookmarks, R.filter(filterBookmark(inputFilter))),
)
 ...
 * weight.
 */
export const getWeightedUnlimitedFilteredBookmarks = createSelector(
  getUnlimitedFilteredBookmarks,
  getActiveTabHref,
  (bookmarks, activeTabHref) =>
    pipe(
      bookmarks,
 ...
 */
export const getWeightedLimitedFilteredBookmarks = createSelector(
  getWeightedUnlimitedFilteredBookmarks,
  getLimitNumRendered,
  (bookmarks, limitNumRendered) =>
    pipe(
      bookmarks,
 ...
 */
export const getNumFilteredUnrenderedBookmarks = createSelector(
  getUnlimitedFilteredBookmarks,
  getWeightedLimitedFilteredBookmarks,
  (us, ls) => Math.max(0, R.size(us) - ls.length),
)

export const getFocusedBookmark = createSelector(
  getWeightedLimitedFilteredBookmarks,
  getFocusedBookmarkIndex,
  (bookmarks, focusedId) =>
    pipe(
      focusedId,
...
export const getBookmarkToEdit = createSelector(
  getBookmarks,
  getBookmarkEditId,
  (bookmarks, editId) =>
    pipe(
...
export const getBookmarkToDelete = createSelector(
  getBookmarks,
  getBookmarkDeleteId,
  (bookmarks, deleteId) =>
    pipe(
...
export const getSortedStagedGroups = createSelector(
  getStagedGroups,
  flow(A.sort(ordStagedBookmarksGroup), A.reverse),
)

export const getStagedGroupToEdit = createSelector(
  getStagedGroups,
  getStagedGroupEditId,
  (groups, editId) =>
    pipe(
      editId,
      O.chain(eid =>
        A.findFirst<StagedBookmarksGroup>(grp => grp.id === eid)(groups),
      ),
    ),
)
 ...
 */
export const getStagedGroupToEditWeightedBookmarks = createSelector(
  getStagedGroupToEdit,
  getActiveTabHref,
  (group, activeTabHref) =>
    pipe(group, O.map(flow(bookmarks.get, A.map(withWeight(activeTabHref))))),
)

export const getStagedGroupBookmarkToEdit = createSelector(
  getStagedGroupToEdit,
  getStagedGroupBookmarkEditId,
  (group, editId) =>
    pipe(
      OT.optionTuple(group, editId),
      O.chain(([{ bookmarks }, eid]) =>
        A.findFirst((bm: LocalBookmark) => bm.id === eid)(bookmarks),
      ),
    ),
)
