...
const bookmarksReducer = curryReducer<BookmarksActions, BookmarksState>(
  a => _s => {
    switch (a.type) {
      case BookmarksActionTypes.SetAllBookmarks:
        return pipe(
          a.payload,
          fromArray(flow(id.get, S.fromNumber)),
          bookmarks.set,
        )
      ...
      case BookmarksActionTypes.UpdateStagedBookmarksGroupBookmark: {
        ...
        return stagedBookmarksGroups.modify(
          mapByPredicate(grpBms.modify(mapByPredicate(constant(newBm))(eqBm)))(
            eqGrp,
          ),
        )
      }
      ...
      case BookmarksActionTypes.DeleteStagedBookmarksGroupBookmark: {
        ...
        return stagedBookmarksGroups.modify(
          mapByPredicate(grpBms.modify(A.filter(not(eqBm))))(eqGrp),
        )
      }
