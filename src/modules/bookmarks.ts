...
const ordTitle: Ord<LocalBookmarkUnsaved> = contramap<
  string,
  LocalBookmarkUnsaved
>(title.get)(ordString)
const ordWeight: Ord<LocalBookmarkWeighted> = contramap<
  URLMatch,
  LocalBookmarkWeighted
>(weight.get)(ordURLMatch)
// The ordering of bookmark weight should be inverted for the UI
const ordWeightForUI: Ord<LocalBookmarkWeighted> = fromCompare(
  flow(ordWeight.compare, invert),
)
export const ordLocalBookmarkWeighted = getMonoid<LocalBookmarkWeighted>().concat(
  ordWeightForUI,
  ordTitle,
)
...
export const filterBookmark = (
  test: ParsedInputResult,
): Predicate<LocalBookmark> => (bookmark): boolean => {
  ...
  if (test.tags.some(t => !bookmark.tags.some(tag => includesCI(t)(tag))))
    return false
...
export function untransform(
  bookmark: LocalBookmark | LocalBookmarkUnsaved,
  ...
  const transformed: RemoteBookmark | RemoteBookmarkUnsaved =
    "id" in bookmark ? { ...base, id: bookmark.id } : base

  return transformed
}

export const formatStagedBookmarksGroupTitle = (
  group: StagedBookmarksGroup,
): string =>
  `${group.bookmarks.length} bookmark${
    group.bookmarks.length === 1 ? "" : "s"
  }, ${formatDistanceToNow(group.time)} ago`
