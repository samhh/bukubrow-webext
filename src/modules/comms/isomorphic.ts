...
const isIsomorphicMessage: Refinement<unknown, IsomorphicMessage> = (
  x: unknown,
): x is IsomorphicMessage =>
  pipe(values(IsomorphicMessage), A.elem(eqStrict)(x))

export const sendIsomorphicMessage = (
  x: IsomorphicMessage,
): TaskEither<Error, void> => pipe(sendMessage(x), TE.map(constVoid))

export const listenForIsomorphicMessages = (
  f: (m: IsomorphicMessage) => void,
): IO<void> => (): void => {
