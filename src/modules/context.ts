...
import {
  ...
  getAllTabs,
  getActiveWindowTabs,
  saveStagedBookmarksAsNewGroupToLocalStorage,
} from "~/modules/comms/browser"
import { values } from "fp-ts-std/Record"
import { eqStrict } from "fp-ts/lib/Eq"
import { runTask, runIOs_, runIO } from "~/modules/fp"

const createContextMenuEntry = (
  x: Menus.CreateCreatePropertiesType,
): IO<void> => (): void => void browser.contextMenus.create(x)

const isContextMenuEntry: Refinement<unknown, ContextMenuEntry> = (
  x,
): x is ContextMenuEntry => pipe(values(ContextMenuEntry), A.elem(eqStrict)(x))
...
export const sendTabsToStagingArea = (
  tabs: NonEmptyArray<SufficientTab>,
): TaskEither<Error, void> =>
  saveStagedBookmarksAsNewGroupToLocalStorage(
    NEA.nonEmptyArray.map(tabs, tab => ({
      title: tab.title,
      desc: "",
      url: tab.url,
      tags: [],
...
const isSufficientTab: Refinement<Tabs.Tab, Tabs.Tab & SufficientTab> = (
  tab,
): tab is Tabs.Tab & SufficientTab => !!tab.title && !!tab.url

const sufficientTabExact: Endomorphism<SufficientTab> = (
  tab: SufficientTab,
) => ({ title: tab.title, url: tab.url })

const sufficientTabsExact = flow(
  A.filter(isSufficientTab),
  A.map(sufficientTabExact),
)

const createContextMenuListener = <A>(g: (a: A) => IO<void>) => (
  f: (x: Menus.OnClickData) => TaskOption<A>,
): IO<void> => (): void =>
...
export const contextClickTabs = (u: Option<string>) => (
  c: ContextMenuEntry,
): TaskOption<NonEmptyArray<SufficientTab>> => {
  switch (c) {
 ...
 */
export const initContextMenusAndListen = (
  cb: (tabs: NonEmptyArray<SufficientTab>) => IO<void>,
): IO<void> => (): void =>
  runIOs_(
